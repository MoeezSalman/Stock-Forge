...
app.get("/api/portfolio", async (req, res) => {
  try {
    ...
    positions.forEach((p) => {
      p.allocPct = totalValue > 0 ? Math.round((p.rawValue / totalValue) * 100) : 0;
      delete p.rawValue;
      delete p.rawDayPnL;
    });
...
app.get("/api/model_analytics/:ticker", async (req, res) => {
  try {
    const ticker = req.params.ticker.toUpperCase();
    const doc = await db.collection("model_analytics").findOne(
      { ticker },
      { projection: { _id: 0 } }
    );
    if (!doc) {
      return res.status(404).json({ error: `No model analytics for ${ticker}. Run the pipeline first.` });
    }
    res.json(doc);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get("/api/model_analytics", async (req, res) => {
  try {
    const docs = await db
      .collection("model_analytics")
      .find({}, { projection: { _id: 0, ticker: 1, model_stats: 1, trained_at: 1 } })
      .toArray();
    res.json(docs);
  } catch (err) {
    res.status(500).json({ error: err.message });
